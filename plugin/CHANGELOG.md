...
- Spring Boot `ArmeriaSettings` completion for YAML and `application.properties` (including `docs-path` / `health-check-path` / `metrics-path` and `internal-services.include` values), plus jump from the Spring Boot Config explorer to `ArmeriaServerConfigurator` / `DocServiceConfigurator` / `HealthCheckServiceConfigurator` / `MetricCollectingServiceConfigurator` beans.
