...
- Armeria annotated HTTP routes and `Server.builder()` registrations appear in the Endpoints tool window (Framework = Armeria, Type = HTTP Server), including HTTP Client URL targets. Prefix and `serviceUnder` paths are prefix URL targets. Spring MVC mappings stay on Spring’s provider.
