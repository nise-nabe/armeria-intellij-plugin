...
- Route Explorer classifies `Server.builder().service()` / `serviceUnder()` targets from resolved types, so `DocService`, `PrometheusExpositionService`, `GrpcService.builder().addService()`, `FileService`, and servlet mounts show the correct protocol and Open DocService / duplicate-index behavior.
