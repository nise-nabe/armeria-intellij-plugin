...
- Route Explorer groups services by virtual host (default host vs `virtualHost("foo.com")`) and lists listen ports from `Server.builder().http` / `.https` / `.port` (including HTTP+HTTPS and PROXY unification). Double-click a virtual-host group to jump to the `virtualHost(...)` call.
