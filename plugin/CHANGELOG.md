...
- Armeria run configurations resolve the listen port from `Server.builder().http` / `.https` / `.port` in the selected main class, or from Spring `armeria.ports[0].port` when the main has no programmatic bind. After start they print DocService / health / metrics URLs (`http://127.0.0.1:<port>/…`) and write `.idea/httpRequests/http-client.env.json` (`host` / `port`) so Generate HTTP Request can use `{{host}}:{{port}}`. Optional checkboxes add `-Dcom.linecorp.armeria.verboseResponses=true` and `-Dcom.linecorp.armeria.reportBlockedEventLoop=true`, and can open DocService in the browser. Missing ports skip URL hints and browser open instead of failing the run.
