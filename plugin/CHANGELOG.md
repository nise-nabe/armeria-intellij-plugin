...
- Annotated-service editor support for remaining annotations: Ctrl/Cmd-click and implementor completion for `@ExceptionHandler` / `@RequestConverter` / `@ResponseConverter` / `@Decorator` (qualified class literals resolve to the named type), media-type completion for `@Produces` / `@Consumes`, Route Explorer distinction and Generate HTTP Request query params for method- and class-level `@MatchesParam`, `@ProducesBinary` as `application/binary`, `@Description` comments in generated `.http` files, `@Default` explorer hints, `@Attribute` name completion, and a weak warning when annotated-service Javadoc/KDoc contains `@param` / `@return` / `@throws` without `armeria-annotation-processor` (including a version-catalog mention in `libs.versions.toml`).
