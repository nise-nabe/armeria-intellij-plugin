...
- Armeria Clients explorer discovers `RestClient` and `BlockingWebClient` factories and conversions (`WebClient.asRestClient()`, `WebClient.blocking()`), labels additional client decorators (`OAuth2Client`, Auth, Throttling, Decoding/Encoding), surfaces EndpointGroup kinds (DNS, ZooKeeper, Eureka, Consul, health-checked), and jumps to overlapping Armeria Services routes (toolbar and gutter; reverse jump from Route Explorer).
