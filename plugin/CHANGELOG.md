...
- Annotated-service editor support for remaining annotations: Ctrl/Cmd-click and implementor completion for `@ExceptionHandler` / `@RequestConverter` / `@ResponseConverter` / `@Decorator`, media-type completion for `@Produces` / `@Consumes`, Route Explorer distinction and Generate HTTP Request query params for `@MatchesParam`, `@Description` comments in generated `.http` files, `@Default` explorer hints, `@Attribute` name completion, and a weak warning when `@Description` or route Javadoc is present without `armeria-annotation-processor`.
