...
- Test helper resolution and blocking-client inspection no longer guess which `ServerExtension` applies when a test class declares multiple `@RegisterExtension` fields; inherited extensions are still resolved when the call names a single matching receiver.
