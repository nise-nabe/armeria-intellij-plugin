...
When `git diff --cached --name-only -- '*.kt' '*.kts' '.editorconfig'` is non-empty, before each `git commit` run `gradle_run_tasks` with `["ktlintCheck"]` (`background: true` + poll `gradle_get_build_status` until terminal success). Fix failures with `gradle_run_tasks` `["ktlintFormat"]` (same poll) or manual edits, `git add` the changed files, re-run until clean. Wait for any in-flight MCP build to finish or cancel it first. Shell fallback: `./gradlew ktlintCheck` / `./gradlew ktlintFormat` (then `git add`). Omit ktlint when the staged index contains none of those paths. See `AGENTS.md` **Commit workflow (coding agents)** for `build-logic` caveats.
