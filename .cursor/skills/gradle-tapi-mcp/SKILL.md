...
2. JAR が利用可能であること（このリポジトリでは `.cursor/install.sh` が GitHub リリースから `~/.local/share/gradle-tapi-mcp-server/gradle-tapi-mcp-server.jar` を取得）
3. 接続前に `gradle_connection_status` で `connected: true` を確認。未接続なら `gradle_connect` または `GRADLE_PROJECT_DIR=${workspaceFolder}`

グローバル設定では `${workspaceFolder}` が開いている Gradle プロジェクトを指す。設定例は [gradle-tapi-mcp-server README](https://github.com/nise-nabe/gradle-tapi-mcp-server/blob/main/README.md) を参照。

## 複数プロジェクト

1 つの MCP プロセスで複数 Gradle プロジェクトを同時に扱える。

1. 各 repo で `gradle_connect`（他プロジェクトの接続は維持される）
2. ツール呼び出しに `projectDirectory` を指定（省略時は `GRADLE_PROJECT_DIR`）
3. `gradle_connection_status` 引数なし → `connections[]` と `defaultProjectDirectory`
4. 別プロジェクトへの同時バックグラウンドビルドは `projectDirectory` 付きで `background: true`
...
| `gradle_get_build_environment` | Gradle/Java 実行環境（`versionInfo` は Gradle 9.4+ で `gradle --version` 相当テキスト） |
...
```json
{ "tasks": [":app:test"], "includePatterns": ["com.example.*"] }
```
