...
- **Kotlin code intelligence:** `.github/lsp.json` configures [fwcd/kotlin-language-server](https://github.com/fwcd/kotlin-language-server) for `.kt` / `.kts` files. The server is installed by `copilot-setup-steps` via `.github/scripts/install-kotlin-language-server.sh`.
