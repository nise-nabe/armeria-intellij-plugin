...
**Use MCP for all Gradle tasks.** Fall back to shell `./gradlew` only when MCP is unresponsive, returns `BUILD_QUEUE_FULL` that cannot be cancelled, or for CI parity — **not** to read compile/test output from a build MCP already ran (see **On MCP build failure** below). Do **not** treat `BUILD_ALREADY_RUNNING` as a reason to shell — retry with `background: true` to enqueue, or poll/cancel `error.activeBuildId`.
...
Use `background: true`, then poll `gradle_get_build_status` for runs that may exceed ~30s (`build`, `:plugin:test`, `:plugin-route-analysis:test`, cold start). `queueIfBusy` defaults to `true` when `background` is true (omit unless setting `false`). `gradle_run_tests` requires selectors (`testClasses` / `testMethods` / `includePatterns`); a whole suite is `gradle_run_tasks`. Selected tests (including `:plugin:test`) use `gradle_run_tests` with `taskPath` + selectors.

## On MCP build failure

Re-poll the **same `buildId`** — do **not** shell `./gradlew` just to read compiler or test output:

- Compile / task failure: read capped `problems` (included by default when the Problems API emitted them). Re-poll with `includeProblems: true` only if `problems` is missing (also when `error` is the TAPI wrapper `Could not execute build using connection…` and `failedTasks` is non-empty)
...
Only **one** MCP build per `projectDirectory` at a time (gate releases immediately on terminal status). `BUILD_ALREADY_RUNNING` includes `error.activeBuildId` for direct polling. With `background: true`, `queueIfBusy` defaults true and the call enqueues (queue depth max 3). Batch multiple test classes/methods in a **single** `gradle_run_tests` instead of parallel MCP calls. To run both `:test` and a custom `JvmTestSuite` (`fastTest`) in one build, use `tasks: [":mod:test", ":mod:fastTest"]` + `includePatterns`. Use `gradle_cancel_build` + poll when you need to stop a stale run (`not_running` means the build already finished).

Do **not** run MCP `gradle_run_tests` and shell `./gradlew :plugin:test` concurrently (IntelliJ test sandbox contention). In this multi-project repo, prefer explicit `taskPath` or `tasks` (e.g. `taskPath: ":plugin-route-analysis:test"`). Unscoped `testClasses`/`testMethods` auto-infer when unambiguous; otherwise `INVALID_ARGUMENT` includes `suggestedTaskPaths` and a `hint`. Selected `:plugin` tests: `gradle_run_tests` `{ "taskPath": ":plugin:test", "testClasses": ["FQCN"], "background": true }`.
...
| One or more test classes/methods (route modules) | `gradle_run_tests` `{ "taskPath": ":plugin-route-analysis:test", "testMethods": { "FQCN": ["method"] }, "background": true }` — batch in one call |
| Plugin fixture tests (whole suite) | `gradle_run_tasks` `{ "tasks": [":plugin:test"], "background": true }` |
| Selected `:plugin` tests | `gradle_run_tests` `{ "taskPath": ":plugin:test", "testClasses": ["FQCN"], "background": true }` |
...
4. Verify tests via MCP (`gradle_run_tests` with `taskPath` + selectors for selected tests including `:plugin:test`; `gradle_run_tasks` for whole suites; `background: true` + poll). On `status: failed`, follow **On MCP build failure** — do not shell `./gradlew` for logs.
