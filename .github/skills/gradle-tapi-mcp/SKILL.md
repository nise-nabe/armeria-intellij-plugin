...
3. Before each `git commit` when staged Kotlin or `.editorconfig` is present (see detection command above), run `gradle_run_tasks` with `["ktlintCheck"]` (`background: true` + poll). On failure, apply `gradle_run_tasks` `["ktlintFormat"]` or manual fixes, `git add` the changed files, and re-run until clean. Wait for in-flight MCP builds to finish or cancel them (`gradle_cancel_build`). Root `ktlintCheck` does not cover `build-logic/` or `settings.gradle.kts`.
